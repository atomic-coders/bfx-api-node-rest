...
class RESTv2 {
   ...
   * @return {Array[]} merged arr of currencies and names sorted with no pairs repeated
   * @private
   */
  _mergeCurrenciesListSymLabel (data) {
    ...
    Object.keys(allCurrObj).forEach((key) => {
      allCurArr.push([key, allCurrObj[key]])
    })
  ...
  currencies (cb = () => {}) {
    ...
    return this._makePublicRequest(url, cb, (data) => {
      const res = this._mergeCurrenciesListSymLabel(data)
