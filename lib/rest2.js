...
class RESTv2 {
  ...
  _genCurrencyList (data) {
    ...
    Object.keys(allCurrObj).forEach((key) => {
      ...
      allCurArr.push([key, cName, cPool, cExpl])
    })
   ...
   */
  tickersHistory (symbols = [], start, end, limit = 2500, cb) {
    const transformer = (data) => {
      return data.map(ticker => (
         (ticker[0] || '')[0] === 't'
   ...
   */
  walletsHistory (end = Date.now(), cb) {
    return this._makeAuthRequest('/auth/r/wallets/hist', { end }, cb, WalletHist)
  }
  ...
  * @return {Promise} p
   ...
   */
  positionsHistory (start = 0, end = Date.now(), limit = 500, cb) {
  ...
  * @see https://docs.bitfinex.com/v2/reference#rest-auth-positions-audit
   */
  positionsAudit (id = [], start = 0, end = Date.now(), limit = 1250, cb) {
